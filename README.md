...
The JSON schema is defined in [OSCEM](https://github.com/osc-em/oscem-schemas/) and the PDBx/mmCIF format in ([PDBML](https://mmcif.wwpdb.org/dictionaries/ascii/mmcif_pdbx_v50.dic) Schema v50 ).
...
* with `--conversions` specify path to [conversions table](https://github.com/osc-em/oscem-converter-extracted/blob/main/csv/pdb_conversions.csv). This table includes correspondance in names between OSC-EM and PDBx
...
```
go get github.com/osc-em/oscem-converter-mmcif
```
and then run it with some of the

```import parser "github.com/osc-em/oscem-converter-mmcif/parser"
    ...
...
Here no checks against [OSCEM schema](https://github.com/osc-em/oscem-schemas) are implemented, it is assumed metadata validates against this schema.  Validation might still be added in the next releases.
